  ...
  window.addEventListener("keydown", (e) => {
    if ($suggest.style.display !== "none") {
      ...
      if (key === "ArrowUp") {
        ...
        this.setState({
          ...this.state,
          cursor: nextCursor < 0 ? this.state.keywords.length - 1 : nextCursor, // 커서 순환
        });
      } else if (key === "ArrowDown") {
        const nextCursor = this.state.cursor + 1;
        this.setState({
          ...this.state,
          cursor: nextCursor > this.state.keywords.length - 1 ? 0 : nextCursor, // 커서 순환
        });
      } else if (key === "Enter") {
